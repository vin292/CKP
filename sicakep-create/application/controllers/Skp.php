...
class Skp extends CI_Controller {
  ...
  public function index()
  {
		...
		$data['skp']=$this->skp->get_skp_bynip($this->session->userdata('niplama'))->result();
		$data['skp_row']=$this->skp->get_skp_bynip($this->session->userdata('niplama'))->num_rows();
...
public function input_skp(){
	...
	$data['page']='skp/skpnew_input_view';
	$this->load->vars($data);
	$this->load->view('template/layout');
}
...
public function simpan_skp(){
		...
		if($number > 0)
		{
				...
				$data = array(
					...
					'tgl_buat' => date('Y-m-d'),
					'tahun' => $this->input->post('tahun_skp'),
					);
